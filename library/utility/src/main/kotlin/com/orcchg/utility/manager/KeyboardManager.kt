package com.orcchg.utility.manager
 ...
 *
 * @see https://medium.com/@munnsthoughts/detecting-if-the-android-keyboard-is-open-using-kotlin-rxjava-2-8aee9fae262c
 */
class KeyboardManager(private val activity: Activity) {
     ...
     */
    fun status(): Observable<KeyboardStatus> =
        Observable.create<KeyboardStatus> { emitter ->
            ...
            emitter.setCancellable { rootView.viewTreeObserver.removeOnGlobalLayoutListener(globalLayoutListener) }
        }.distinctUntilChanged()
}

enum class KeyboardStatus { OPEN, CLOSED }
